...
import imi.scene.utils.traverser.PPolygonMeshAssemblingProcessor;
...
//            ex.printStackTrace();
    ...
    private final FastTable<PColladaEffect>         m_EffectList = new FastTable<PColladaEffect>();
    // Material mapping and list
    private final FastMap<String, ColladaMaterial>  m_MaterialMap = new FastMap<String, ColladaMaterial>();
    private final FastTable<ColladaMaterial>        m_MaterialList = new FastTable<ColladaMaterial>();
    ...
    private final FastTable                                   m_ColladaMaterialInstances = new FastTable();
    ...
    private final FastTable<PPolygonMesh>    m_PolygonMeshes = new FastTable<PPolygonMesh>();

    //  Contains the PolygonSkinnedMeshes loaded.
    private final FastTable<PPolygonSkinnedMesh> m_PolygonSkinnedMeshes = new FastTable<PPolygonSkinnedMesh>();
    private final FastTable<PColladaSkin>        m_ColladaSkins = new FastTable<PColladaSkin>();
    private final FastTable<PColladaNode>        m_ColladaNodes = new FastTable<PColladaNode>();
    private final FastTable<PColladaNode>        m_FactoryColladaNodes = new FastTable<PColladaNode>();
    
    private final FastTable<PColladaAnimatedItem>     m_ColladaAnimatedItems = new FastTable<PColladaAnimatedItem>();
    private final FastTable<PColladaCameraParams>     m_ColladaCameraParams = new FastTable<PColladaCameraParams>();
    private final FastTable<PColladaCamera>           m_ColladaCameras = new FastTable<PColladaCamera>();
     ...
     * @param colladaFile URL to the collada file to load
     * @return True on success, false on soft failure
     * @throws IOException If a connection cannot be opened to the provided URL
     * @throws ColladaLoadingException If something goes wrong with the validation process
     */
    public boolean load(PScene loadingPScene, URL colladaFile) throws IOException, ColladaLoadingException {
...
//            logger.info("Root ColladaNode '" + node.getName() + "', isJoint " + ((node.isJoint()) ? "yes" : "no"));
    ...
    public void applyConfiguration(ColladaLoaderParams params)
    {
        ...
        {
            setLoadRig(params.m_bLoadSkeleton);
            setLoadGeometry(params.m_bLoadGeometry);
            setLoadAnimations(params.m_bLoadAnimation);
            setPrintStats(params.m_bShowDebugInfo);
            m_MaxNumberOfWeights = params.m_nMaxWeights;
        }
