...
public class ThreadedTreeTraverser extends Thread 
{
    ...
    private TraversalType m_TraversalType = TraversalType.BREADTH_FIRST; // How should the tree be traversed?
     ...
     */
    public ThreadedTreeTraverser(PNode root, NodeProcessor processor, TraversalType TraversalMethod)
    {
        m_RootNode = root;
        m_Processor = processor;
    }
    ...
    public void run()
    {
        while (true) // Exit conditions at the end of the loop
        {
            ...
            boolean bDaemonize = false;
            synchronized(this) {
                bDaemonize = m_bDaemonize;
            }
            if (m_bDie == true || bDaemonize == false) // Check against local copy
            {
                output("Exiting loop");
                break;
            }
     ...
     */
    public synchronized void setTraversalMethod(TraversalType TraversalMethod)
    {
     ...
     */
    public synchronized NodeProcessor getProcessor()
    {
        return m_Processor;
    }
     ...
     * No reference type should be modified, these accessors are provided for
     * reading values only. To modify settings, use the synchronized set methods.
     * @return m_TraversalType (TraversalType)
     */
